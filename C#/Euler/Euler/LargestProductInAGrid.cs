...
#pragma warning restore CS0660 // 类型定义运算符 == 或运算符 !=，但不重写 Object.Equals(object o)
    ...
    public class LargestProductInAGrid
    {
        ...
        void FindProduct(int x, int y,DiagonallyWay way = DiagonallyWay.None)
        {
            ...
            if (vectorList.Count == targetNumber)
            {
                int tempProduct = 1;
                foreach (Vector vector in vectorList)
                {
                    tempProduct *= grid[vector.x, vector.y];
                }
            ...
            //只需要对角线不需要其他类型
            //如果需要其他类型，只需要增加几行- -各种方向，然后去掉way
            if (way == DiagonallyWay.Left) //左下
            {
                FindProduct(x - 1, y + 1, way);
            }

            if (way == DiagonallyWay.Right) //右下
            {
                FindProduct(x + 1, y + 1, way);
            }

            if (way == DiagonallyWay.None)
            {
                FindProduct(x - 1, y + 1, DiagonallyWay.Left);
                FindProduct(x + 1, y + 1, DiagonallyWay.Right);
            }
        ...
        bool CheckIndex(Vector vector)
        {
            ...
            //是否超出边界
            return vector.x >= 0 && vector.x <= grid.GetUpperBound(0) && vector.y >= 0 && vector.y <= grid.GetUpperBound(1);
        }
    }
}
